source "vmware-iso" "almalinux" {
  boot_command = [
    "<up><wait><tab> text ks=http://{{ .HTTPIP }}:{{ .HTTPPort}}/${var.kickstart}<enter>",
  ]
...
source "virtualbox-iso" "almalinux" {
  ...
  vboxmanage = [
    ...
    ["modifyvm", "{{.Name}}", "--paravirtprovider", var.virtualbox_paravirtprovider],
    ["modifyvm", "{{.Name}}", "--nictype1", var.virtualbox_nictype],
  ]
  virtualbox_version_file = ".vbox_version"
  vm_name                 = var.vm_name
}

source "parallels-iso" "almalinux" {
  boot_command = [
    "<up><wait><tab> text ks=http://{{ .HTTPIP }}:{{ .HTTPPort}}/${var.kickstart}<enter>",
  ]
  disk_size              = var.disk_size
